...
“多播 Observable”通过可能有许多订阅者的 Subject 来传递通知，而普通的“单播 Observable”仅向单个 Observer 发送通知。
...
<span class="informal">多播的 Observable 在底层使用 Subject 来让多个 Observer 看到相同的 Observable 执行过程。</span>
...
如果我们希望避免显式调用 `connect()`，可以使用 ConnectableObservable 的 `refCount()` 方法（引用计数），它返回一个 Observable 来跟踪它有多少订阅者。当订阅者数量从 `0` 增加到 `1` 时，它会为我们调用 `connect()`，从而开始共享执行。只有当订阅者数量从 `1` 减少到 `0` 时，才会完全退订，并停止进一步执行。
...
<span class="informal">`refCount` 能使多播的 Observable 在第一个订阅者抵达时自动开始执行，并在最后一个订阅者离开时停止执行。</span>
...
`ReplaySubject` 与 `BehaviorSubject` 类似，它可以将旧值发送给新订阅者，但它也可以*记录* Observable 执行结果的一部分。
...
<span class="informal">`ReplaySubject` 会记录来自 Observable 执行的多个值，并将它们重播给新订阅者。</span>
...
除了缓冲区大小之外，你还可以指定一个以毫秒为单位的*窗口时间*，以确定记录的值可以存在多长时间。在以下示例中，我们使用 `100` 个元素的大型缓冲区，但窗口时间参数仅为 `500` 毫秒。
...
AsyncSubject 类似于 [`last()`](/api/operators/last) 操作符，因为它会等待 `complete` 通知以传递单个值。
