...
**什么是调度器？**调度器控制订阅何时开始以及何时传递通知。它由三个组件组成。
...
<span class="informal">调度器允许你定义 Observable 将在什么执行上下文中向其观察者传递通知。</span>
...
`async` 调度器使用 `setTimeout` 或 `setInterval` 运行，即使给定的 `delay` 为零。像往常一样，在 JavaScript 中，已知 `setTimeout(fn, 0)` 在下一次事件循环迭代中最早运行函数 `fn`。这解释了为什么 `just after subscribe` 发生后将 `got value 1` 传递给 `finalObserver`。
...
| `null` | 通过不传递任何调度器，通知以同步和递归方式传递。将此用于恒定时间操作或尾递归操作。|
...
因为 RxJS 使用最少的并发调度器，如果你想为了性能目的引入并发，你可以选择一个不同的调度器。要指定特定的调度器，你可以使用那些采用调度器的操作符方法，例如 `from([10, 20, 30], asyncScheduler)`。
...
**静态创建操作符通常以 Scheduler 作为参数。**例如，`from(array, scheduler)` 允许你指定在传递从 `array` 转换的每个通知时要使用的调度器。它通常是操作符的最后一个参数。以下静态创建操作符采用 Scheduler 参数：
...
**使用 `subscribeOn` 来安排 `subscribe()` 调用将在什么上下文中发生。**默认情况下，对 Observable 的 `subscribe()` 调用将同步并立即发生。但是，你可以使用实例操作符 `subscribeOn(scheduler)` 延迟或安排在给定调度器上发生的实际订阅，其中 `scheduler` 是你提供的参数。
...
请注意，`cache` 和 `publishReplay` 都接受 Scheduler，因为它们使用了 ReplaySubject。ReplaySubjects 的构造函数将可选的 Scheduler 作为最后一个参数，因为 ReplaySubject 可能会处理时间，这仅在 Scheduler 的上下文中才有意义。默认情况下，ReplaySubject 使用 `queue` 调度器提供时钟。
