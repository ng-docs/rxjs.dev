 ...
 *
 * 注意：这将为所有 TypeScript 用户全局添加 Symbol.observable，但是，我们不会再填充（polyfill） Symbol.observable
 *
 */
declare global {
...
export interface Timestamp<T> {
   ...
   *
   * 时间戳。默认情况下，这是以纪元毫秒数为单位。可能会因传给操作符的时间戳提供者而异。
   *
   */
  timestamp: number;
}
 ...
 *
 * 表示来自 Observable 的“下一个”的通知。可以与 {@link dematerialize} 一起使用。
 *
 */
 ...
 * Can be used with {@link dematerialize}.
 *
 * 表示来自 Observable 的“错误”的通知。可以与 {@link dematerialize} 一起使用。
 *
 */
 ...
 * Can be used with {@link dematerialize}.
 *
 * 表示来自 Observable 的“完成”的通知。可以与 {@link dematerialize} 一起使用。
 *
 */
...
export interface TimestampProvider {
   ...
   *
   * `ReplaySubject` 类的类型或 `timestamp` 之类的操作符会使用它来计算事件之间传递的时间量。
   *
   */
  now(): number;
}
 ...
 *
 * 从 `ObservableInput<any>` 中提取类型。如果你现有 `O extends ObservableInput<any>` 并传入 `Observable<number>` 或 `Promise<number>` 等，其类型就会是 `number`。
 *
 */
 ...
 *
 * 从 `ObservableInput<any>[]` 中提取元素类型的联合。如果你现有 `O extends ObservableInput<any>[]` 并传入 `Observable<string>[]` 或 `Promise<string>[]`，就会得到一个 `string` 类型。如果你传入 `[Observable<string>, Observable<number>]`，就会得到一个类型 `string | number`。
 *
 */
 ...
 * of `[string, number]`.
 *
 * 从 `ObservableInput<any>[]` 中提取元素类型的元组。如果你有 `O extends ObservableInput<any>[]` 并且传入 `[Observable<string>, Observable<number>]`，就会得到一个类型 `[string, number]`。
 *
 */
 ...
 * et al.
 *
 * 用于从传给 {@link forkJoin} 等函数的参数推断出类型。这样你就可以拥有 `forkJoin([Observable<A>, PromiseLike<B>]): Observable<[A, B]>` 等。
 *
 */
 ...
 *
 * 在头部构造一个具有指定类型的新元组。如果你声明 `Cons<A, [B, C]>` 就会返回 `[A, B, C]`。
 *
 */
 ...
 *
 * 提取元组的头部。如果你声明 `Head<[A, B, C]>` 就会返回 `A`。
 *
 */
 ...
 *
 * 提取元组的尾部。如果你声明 `Tail<[A, B, C]>` 就会返回 `[B, C]`。
 *
 */
 ...
 *
 * 从 Array 类型中提取泛型值。如果你有 `T extends Array<any>` 并将 `string[]` 传给它，`ValueFromArray<T>` 就将返回 `string` 的实际类型。
 *
 */
 ...
 *
 * 如果可能，就从 {@link ObservableNotification} 获取值类型。
 *
 */
 ...
 *
 * 一种表示“假”值域的简单类型......有一个明显的例外：`NaN` 是“假值”，但是，它不是、也不能使用 TypeScript 来定义类型。在此处查看评论： <https://github.com/microsoft/TypeScript/issues/28682#issuecomment-707142417>
 *
 */
 ...
 *
 * 一个基本签名，RxJS 将使用它来寻找标识和使用 [ReadableStream](https://streams.spec.whatwg.org/#rs-class) 作为 {@link ObservableInput} 源。
 *
 */
 ...
 *
 * 一个带有 `connect` 方法的 Observable，用于创建对底层源的订阅，通过多播将其与所有消费者连接起来。
 *
 */
export interface Connectable<T> extends Observable<T> {
   ...
   *
   *（幂等）调用此方法将通过底层 {@link Subject} 将底层源 Observable 连接到所有已订阅的消费者。
   *
   ...
   *
   * 退订时，订阅将“断开”源与连接器主体的连接，将这些通知提供给所有消费者。
   *
   */
