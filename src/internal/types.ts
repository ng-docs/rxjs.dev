...
export interface Timestamp<T> {
   ...
   *
   * 时间戳。默认情况下，这是以纪元毫秒为单位。可能会因传给操作符的时间戳提供者而异。
   *
   */
  timestamp: number;
}
 ...
 *
 * 从 Array 类型中提取泛型值。如果你有 `T extends Array<any>` 并将 `string[]` 传给它，`ValueFromArray<T>` 将返回 `string` 的实际类型。
 *
 */
...
export interface Connectable<T> extends Observable<T> {
   ...
   *
   * 退订时，订阅将“断开”源与连接器主体的连接，将通知切断给所有消费者。
   *
   */
