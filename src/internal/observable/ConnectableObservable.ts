...
export class ConnectableObservable<T> extends Observable<T> {
   ...
   * Details: <https://rxjs.dev/deprecations/multicasting>
   *
   * 将在 v8 中删除。使用 {@link connectable} 创建一个可连接的 observable。`new ConnectableObservable(source, factory)` 等价于 `connectable(source, { connector: factory })`。当需要 `refCount()` 方法时，应使用 {@link share} 操作符： `new ConnectableObservable(source, factory).refCount()` 等效于 `source.pipe(share({ connector: factory }))`。详细信息： <https://rxjs.dev/deprecations/multicasting>
   *
   */
