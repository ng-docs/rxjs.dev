 ...
 *
 * 当 `WebSocketSubject` 被订阅时，它会尝试建立一个 Socket 连接，除非已经建立了一个。这意味着许多订阅者将始终在同一个 Socket 上侦听，从而节省资源。但是，如果两个实例由 `WebSocketSubject` 组成，即使这两个实例提供了相同的 url，它们也会尝试建立单独的连接。当 `WebSocketSubject` 的消费者退订时，只有在没有更多订阅者仍在监听的情况下，才会关闭 Socket 连接。如果一段时间后消费者再次开始订阅，则重新建立连接。
 *
 ...
 * will error.
 *
 * 调用 `next` 不会影响 `WebSocketSubject` 的订阅者 - 他们没有任何信息表明某些内容已发送到服务器（当然，除非服务器以某种方式响应消息）。另一方面，由于调用 `complete` 会触发关闭 Socket 连接的尝试。如果该连接在没有任何错误的情况下关闭，则此流将完成，从而通知所有订阅者。而且由于调用 `error` 也会关闭 Socket 连接（只是服务器的状态码不同），如果关闭本身没有错误，则订阅的 Observable 将不会出错（正如人们所期望的那样），但会正常完成。在这两种情况下（调用 `complete` 或 `error`），如果关闭 Socket 连接的过程导致一些错误，*则*流将出错。
 *
 * **Multiplexing**
 *
 * **multiplex（多路复用）**
 ...
 *
 * `WebSocketSubject` 有一个额外的操作符，在其他 Subjects 中没有。它被称为 `multiplex`，它用于模拟打开多个 Socket 连接，而实际上只真正维护一个。例如，一个应用程序既有聊天面板，也有关于体育新闻的实时通知。由于这是两个不同的功能，因此为每个功能设置两个单独的连接是有意义的。也许甚至可以有两个具有 WebSocket 端点的单独服务，在单独的机器上运行，只有 GUI 将它们组合在一起。每个功能都有一个 Socket 连接可能会变得过于昂贵。将单个 WebSocket 端点用作其他服务（在本例中为聊天和体育新闻服务）的网关是一种常见模式。即使客户端应用程序中只有一个连接，也希望能够像处理两个单独的 Socket 一样操作流。这省去了在网关中手动注册和注销给定服务的工作，并过滤掉某些感兴趣的消息。这正是 `multiplex` 方法的用途。
 *
 ...
 *
 * 方法接受三个参数。前两个分别是返回订阅和退订消息的函数。每当结果 Observable 的消费者订阅和退订时，这些消息都会发送到服务器。服务器可以使用它们来验证某种消息是应该开始还是停止转发给客户端。在上面的示例应用程序中，在获得具有适当标识符的订阅消息后，网关服务器决定它要连接到真实的体育新闻服务并开始从中转发消息。请注意，这两条消息都将作为函数返回的内容发送，默认情况下，它们会使用 JSON.stringify 进行序列化，就像通过 `next` 推送的消息一样。另请记住，这些消息会在*每次*订阅和退订时发送。这是潜在的危险，因为 Observable 的一个消费者可能会退订，并且服务器可能会停止发送消息，因为它收到了退订消息。这需要在服务器上处理，或者在从“multiplex”返回的 Observable 上使用 {@link publish}。
 *
 ...
 *
 * `multiplex` 的返回值是一个 Observable，其中包含从模拟 Socket 连接传入的消息。请注意，这不是 `WebSocketSubject`，因此再次调用 `next` 或 `multiplex` 将失败。要将值推送到服务器，请使用根 `WebSocketSubject`。
 *
 * ## Examples
 *
 * ## 例子
