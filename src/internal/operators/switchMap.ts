 ...
 *
 * 返回一个 Observable，该 Observable 基于应用你提供给源 Observable 发出的每个条目的函数来发射条目，其中该函数返回一个（所谓的“内部”）Observable。每次它观察到这些内部 Observable 之一时，输出 Observable 开始发射由该内部 Observable 发射的条目。当一个新的内部 Observable 被发射时，`switchMap` 停止从之前发射的内部 Observable 发射条目，并开始从新的发射条目。对于后续的内部 Observable，它继续表现得像这样。
 *
 * ## Example
 *
 * ## 例子
 ...
 *
 * 一个返回 Observable 的函数，该函数发出将投影函数（和可选的已弃用的 `resultSelector`）应用于源 Observable 发出的每个条目的结果，并且仅从最近投影的内部 Observable 中获取值。
 *
 */
export function switchMap<T, R, O extends ObservableInput<any>>(
  project: (value: T, index: number) => O,
