 ...
 *
 * `onErrorResumeNext` 返回一个 Observable，它从源 Observable 订阅和重新发送值开始。当它的值流结束时——无论 Observable 是否完成或发送错误 `onErrorResumeNext` 将订阅作为参数传递给该方法的第一个 Observable。它也将开始重新发送其值，并且 - 再次 - 当该流结束时，`onErrorResumeNext` 将继续订阅提供的系列中的另一个 Observable，无论之前的 Observable 是否完成或以错误结束。这将一直发生，直到系列中没有更多的 Observable，此时返回的 Observable 将完成 - 即使最后订阅的流以错误结束。
 *
 ...
 * an error.
 *
 * 因此 `onErrorResumeNext` 可以被认为是 {@link concat} 操作符的版本，当涉及到其输入 Observables 发送的错误时，它更加宽容。`concat` 仅在前一个 Observable 成功完成时才订阅系列中的下一个 Observable，`onErrorResumeNext` 订阅即使它以错误结束。
 *
 * Note that you do not get any access to errors emitted by the Observables. In particular do not
 * expect these errors to appear in error callback passed to {@link Observable#subscribe}. If you want to take
 * specific actions based on what error was emitted by an Observable, you should try out {@link catchError} instead.
 *
 * 请注意，你无法访问 Observables 发送的错误。特别是不要期望这些错误出现在传递给 {@link Observable#subscribe} 的错误回调中。如果你想根据 Observable 发送的错误采取特定的行动，你应该尝试 {@link catchError} 代替。
 *
 * ## Example
 *
 * ## 例子
 ...
 *
 * 一个返回 Observable 的函数，该函数从源 Observable 发送值，但是 - 如果它出错 - 订阅下一个传递的 Observable 等等，直到它完成或用完 Observables。
 *
 */
export function onErrorResumeNext<T, A extends readonly unknown[]>(
