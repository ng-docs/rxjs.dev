 ...
 *
 * 使用指定的调度器从源 Observable 重新发出所有通知。
 *
 ...
 *
 * `observeOn` 是一个接受调度器作为第一个参数的操作符，它将用于重新调度源 Observable 发出的通知。如果你无法控制给定 Observable 的内部调度器，但仍想控制其值何时发出，这可能很有用。
 *
 ...
 *
 * 事实上，`observeOn` 接受第二个参数，它以毫秒为单位指定将发出什么延迟通知。{@link delay} 操作符和 `observeOn` 的主要区别在于 `observeOn` 会延迟所有通知——包括错误通知——而 `delay` 会在源 Observable 发出错误时立即传递它。一般来说，强烈建议对流中的任何类型的值延迟使用 `delay` 操作符，同时使用 `observeOn` 指定通常应该使用哪个调度器来发出通知。
 *
 * ## Example
 *
 * ## 例子
 ...
 *
 * 一个返回 Observable 的函数，它发出与源 Observable 相同的通知，但带有提供的调度器。
 *
 */
