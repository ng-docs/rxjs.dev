 ...
 *
 * 返回一个 Observable，该 Observable 基于将你提供的函数应用于源 Observable 发送的每个条目，其中该函数返回一个 Observable，然后合并这些结果 Observable 并发送此合并的结果，从而发送条目。
 *
 * ## Example
 *
 * ## 例子
 ...
 *
 * 一个返回 Observable 的函数，该函数发送将投影函数（以及可选的已弃用的 `resultSelector`）应用于源 Observable 发送的每个条目并合并从此转换获得的 Observables 的结果的结果。
 *
 */
export function mergeMap<T, R, O extends ObservableInput<any>>(
  project: (value: T, index: number) => O,
