...
export interface GlobalConfig {
   ...
   *
   * 如果为 true，则打开同步重新抛出错误的行为，这是 v6 及更高版本中已弃用的行为。这种行为会导致某些不良模式，例如将订阅调用包装在 try/catch 块中。它还会带来生产者干扰，这是一种令人讨厌的错误，其中下游消费者可能会因未处理的错误而破坏所有观察者的多播。除非出于为迁移争取时间的目的，否则请勿使用此标志。
   *
   ...
   * to `subscribe`.
   *
   * 如果为 true，则启用 v5 中未公开的功能：在传给 `subscribe` 的观察者创建的 `next` 函数中通过 `this` 上下文来访问 `unsubscribe()` 的能力。
   *
   ...
   *
   * 从版本 8 开始，RxJS 将不再支持更改作为订阅观察者的一部分提供的 next 函数的上下文。相反，你将有权访问订阅或信号或令牌，从而允许你执行退订和测试关闭状态等操作。将在 v8 中删除。
   *
   */
