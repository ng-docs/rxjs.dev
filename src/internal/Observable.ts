...
export class Observable<T> implements Subscribable<T> {
   ...
   * the thought.
   *
   * `subscribe` 不是一个普通的操作符，而是一个调用 Observable 内部 `subscribe` 函数的方法。例如，它可能是你传给 Observable 构造函数的函数，但大多数时候它是一个库实现，它定义了 Observable 将发送什么以及何时发送。这意味着调用 `subscribe` 实际上是 Observable 开始工作的时刻，而不是它被创建的时刻，因为它通常是这样想的。
   *
