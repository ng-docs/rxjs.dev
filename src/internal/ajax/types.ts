 ...
 *
 * 有效的 Ajax 方向类型。在 {@link AjaxResponse } 对象中为与上传相关的事件的 `type` 添加“upload_”前缀，为与下载相关的事件的 `type` 添加“download_ ”前缀。
 *
 */
...
export interface AjaxRequest {
   ...
   *
   * 是否要带着凭证发送 CORS 请求。如果为 `false`，也会忽略 CORS 响应中的 cookie。
   *
   */
...
export interface AjaxConfig {
   ...
   *
   * 是否异步发送请求。默认为 `true`。如果设置为 `false`，这将阻塞线程，直到收到 AJAX 请求响应为止。
   *
   */
  async?: boolean;
   ...
   *
   * 导致底层 XMLHttpRequest 超时之前的等待时间。这仅在 `async` 配置项尚未设置或设置为 `true` 时才生效。默认为 `0`，这是“永不超时”的惯用语。
   *
   */
  timeout?: number;
   ...
   *
   * 要在 CORS 请求中发送用户凭证，请设置为 `true`。要从 CORS 请求中排除用户凭证，*或者* 当 CORS 响应要忽略 cookie 时，请设置为 `false`。
   *
   * Defaults to `false`.
   *
   * 默认为 `false`。
   ...
   * not `Window`.
   *
   * 可以设置它，以更改响应类型。有效值为 `"arraybuffer"`、`"blob"`、`"document"`、`"json"` 和 `"text"`。请注意，如果全局上下文不是 `Window`，则忽略 `"document"` 的类型（例如 XML 文档）。
   *
   * Defaults to `"json"`.
   *
   * 默认为 `"json"`。
   ...
   *
   * 注意：此 AJAX 实现依赖于兼容 XMLHttpRequest 标准的实现提供的内置序列化和 Content-Type 标头设置，请确保你使用的任何实现都符合该标准。
   *
   */
   ...
   *
   * 对于错误的状态代码，这**不会**出错。相反，它总是在 HTTP 响应返回时*完成*。
   *
   ...
   *
   * 如果你要查找进度事件，请改用 {@link includeDownloadProgress} 和 {@link includeUploadProgress}。这个属性将在 v8 中删除。
   *
   */
   ...
   *
   * 要添加到请求 URL 中的查询字符串参数。*这将需要 Internet Explorer 中 `URL` 和 `URLSearchParams` 的 polyfill！*
   *
   ...
   *
   * 接受查询字符串、`URLSearchParams` 对象、键/值对字典或键/值条目元组数组。（本质上，它需要 `new URLSearchParams` 能正常接收的任何东西）。
   *
   ...
   *
   * 如果由于某种原因在 `url` 参数中有一个查询字符串，这将附加到 url 中的查询字符串，但它也会覆盖任何完全匹配的键的值。换句话说，`/test?a=1&b=2` 的 url 和 `{ b: 5, c: 6 }` 的 queryParams 将导致形如 `/test?a=1&b=5&c=6` 的 url。
   *
   */
  queryParams?:
    | string
