...
export class TestScheduler extends VirtualTimeScheduler {
   ...
   *
   * 指示 TestScheduler 实例是否正在“运行模式”下运行，这意味着它正在处理对 `run()` 的调用
   *
   */
   ...
   *
   * `run` 方法会在“运行模式”下执行测试 —— 该测试中使用的调度器会自动委托给 `TestScheduler`。也就是说，在“运行模式”中，不需要显式地将 `TestScheduler` 实例传给 Observable 的创建者或操作符。
   *
